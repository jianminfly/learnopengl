...
// settings
const unsigned int SCR_WIDTH = 17;
const unsigned int SCR_HEIGHT = 289;
...
int main()
{
    ...
    // ------------------------------------
    Shader lutShader("base.vs", "lut1.fs");
    ...
    unsigned char* data = stbi_load("C:\\Users\\EDZ\\Desktop\\shader editor\\1.png", &width, &height, &nrChannels, 0);
    if (data)
    {
    ...
    // set the texture wrapping parameters
    glTexParameteri(GL_TEXTURE_3D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);	// set texture wrapping to GL_REPEAT (default wrapping method)
    ...
    // The FileSystem::getPath(...) is part of the GitHub repository so we can find files on any IDE/platform; replace it with your own image path.
    unsigned char* data1 = stbi_load("C:\\Users\\EDZ\\Desktop\\shader editor\\lut\\candlelight.png", &width, &height, &nrChannels, 0);

    if (data1)
    {
        glTexImage3D(GL_TEXTURE_3D, 0, GL_RGBA8, 17, 17, 17, 0, GL_RGBA, GL_UNSIGNED_BYTE, data1);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        //uint8_t* output = new uint8_t[SCR_WIDTH * SCR_HEIGHT * 32 + 1];
