...
int main()
{
    ...
    // ------------------------------------
    Shader ourShader("base.vs", "bg-erase-blur.frag");
    ...
    unsigned int texture1;
    {
        ...
        unsigned char* data = stbi_load("C:\\Users\\EDZ\\Desktop\\shader editor\\watermark.PNG", &width, &height, &nrChannels, 4);
        if (data)
        {
    ...
    unsigned int texture2;
    {
        ...
        // The FileSystem::getPath(...) is part of the GitHub repository so we can find files on any IDE/platform; replace it with your own image path.
        unsigned char* data = stbi_load("C:\\Users\\EDZ\\Desktop\\shader editor\\maskwatermark.jpg", &width, &height, &nrChannels, 4);
        if (data)
        {
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, data);//png图像需使用RGBA
            glGenerateMipmap(GL_TEXTURE_2D);
